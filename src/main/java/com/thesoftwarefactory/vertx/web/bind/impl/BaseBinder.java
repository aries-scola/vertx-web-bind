...
public abstract class BaseBinder<T> implements Binder<T> {
	...
	protected boolean isValidPost(RoutingContext routingContext) {
		return routingContext.request().method()==HttpMethod.POST && routingContext.request().isExpectMultipart();
	}
	
	protected String getValue(BindingInfo bindingInfo, RoutingContext routingContext) {
	...
	protected String getValueFromRequests(String name, int index, RoutingContext routingContext) {
		List<String> results = routingContext.request().params().getAll(name);
		return results.size() > index ? results.get(index) : null;
	}

	protected String getValueFromHeader(String name, int index, RoutingContext routingContext) {
		List<String> results = routingContext.request().headers().getAll(name);
		return results.size() > index ? results.get(index) : null;
	}

	protected String getValueFromData(String name, int index, RoutingContext routingContext) {
		Object result = routingContext.data().get(name);
		return result!=null && result instanceof String ? (String) result : null;
	}

	protected String getValueFromForm(String name, int index, RoutingContext routingContext) {
	...
	protected String getValue(String name, int index, RoutingContext routingContext) {
	...
	/* (non-Javadoc)
	 * @see com.thesoftwarefactory.vertx.binders.Binder#toMap(io.vertx.ext.web.RoutingContext, com.thesoftwarefactory.vertx.binders.BindingInfo, java.util.Map)
	 */
	@Override
	public void bindToUrl(BindingInfo bindingInfo, T valueToBind, UriBuilder builder) {
		if (valueToBind!=null) {
			builder.addQueryParameter(bindingInfo.name(), valueToBind.toString());
		}
	}

	protected final static Type typeListOfString = new TypeToken<List<String>>(){}.type();
